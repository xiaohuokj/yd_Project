  ...
  <div class="wrap-main" :style="{height: tableHeight}">
    <cui-table  ref="accountTable" :tableConfigs="tableConfigs">
      <template slot="operate" slot-scope="scope">
        <el-button type="text"  size="small" @click="editDalog(scope.row)">详情</el-button>
      </template>
      <template slot="issueNumber" slot-scope="scope">
        <span>{{scope.row.issueNumber}}</span>
        <span v-if="scope.row.isOverTime==1 && scope.row.timeLimit != null"><el-tag type="danger" color="#fff" :hit=true size="mini">超时</el-tag></span>
        <span v-if="scope.row.isReissued==1 && scope.row.timeLimit != null"><el-tag type="danger" color="#fff" :hit=true size="mini">重办</el-tag></span></span>
      </template>
      //判断问题来源
      ...
      <template slot="primaryClassification" slot-scope="scope">
        <span v-if="scope.row.primaryClassification != null">{{scope.row.primaryClassification}}-{{scope.row.secondClassification}}</span>
      </template>
      <template slot="timeLimit" slot-scope="scope">
        <span v-if="scope.row.timeLimit != null">{{scope.row.timeLimit}} 小时</span>
      </template>
